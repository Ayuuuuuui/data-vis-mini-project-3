...
if st.button("Enter"):
    ...
    # Generate HTML for highlighted NER entities
    highlighted_text_html = render_ner_html(long_text, tags)
    
    # Display the original text with highlighted NER entities
    st.write("#### Output ")
    st.markdown(
        """
        <span style='background-color: #FFB067; border-radius: 5px; padding: 2px;'>O</span>
    ...
    html(highlighted_text_html, height=200)  # Adjust height as needed
## ----------------------------------------------------------------------------------- ##
